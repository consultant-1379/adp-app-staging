...
def defaultReviewGroup = 'deployer-manual-commit-reviewers'
...
pipeline {
    ...
    stages {
        ...
        stage('Checkout deployer') {
            steps {
                script {
                    gitdeployer.checkoutRefSpec("${GERRIT_REFSPEC}", 'FETCH_HEAD', '')
                }
            }
        }
        ...
        stage ('Verify reviewers') {
            ...
            steps {
                script {
                    def result = false
                    def reviewRules = readYaml file: "adp-app-staging/technicals/deployer_reviewers.yaml"
                    try {
                        result = gitdeployer.verifyCodeReviewers("${GERRIT_REFSPEC}", reviewRules)
                    }
                    catch (err) {
                        error err.message
                    }
        ...
        stage('Check if the change can be added to the queue') {
            steps {
                script {
                    if ( gitdeployer.listGerritMembers(defaultReviewGroup).contains("${GERRIT_EVENT_ACCOUNT_EMAIL}") ) {
        ...
        stage('Execute eea_deployer_manual_flow_codereview_ok job') {
            when {
                expression { env.PATCHSET_ALLOWED_TO_MERGE }
            }
            steps {
                script {
                    codereview_ok_job = build job: "eea-deployer-manual-flow-codereview-ok", parameters: [
