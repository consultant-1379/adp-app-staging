...
pipeline {
    ...
    stages {
                    ...
                    data.currentPatchSet.files.each { file_rec ->
                        ...
                        if (file_rec.file  == "bob-rulesets/ruleset2.0.yaml") {
                            withCredentials([usernamePassword(credentialsId: 'git-functional-http-user', usernameVariable: 'GERRIT_USERNAME', passwordVariable: 'GERRIT_PASSWORD')]) {
                                sh 'git fetch https://${GERRIT_USERNAME}:${GERRIT_PASSWORD}@${GERRIT_HOST}/a/EEA/deployer ${GERRIT_REFSPEC} && git cherry-pick FETCH_HEAD'
                            }
                        }
        ...
        stage('Resource locking - Publish DEPLOYER Helm Chart') {
            options {
                lock resource: null, label: "deployer-publish", quantity: 1, variable: "system"
            }
            stages {
        ...
        stage('Publish DEPLOYER package') {
            steps {
                script {
                    // Download the DEPLOYER package from internal repository and then upload it to drop one
                    withCredentials([string(credentialsId: 'arm-eea-eceaart-api-token', variable: 'API_TOKEN_EEA')]) {
                        // Download the DEPLOYER from internal repository
                        sh 'curl -H "X-JFrog-Art-Api: $API_TOKEN_EEA" -fO https://arm.seli.gic.ericsson.se/artifactory/proj-eea-internal-generic-local/eea-deployer-$CHART_VERSION.tar.gz'
                        sh """
                            set -x
                            ls -la
                            tar xzvf "eea-deployer-\${CHART_VERSION}.tar.gz"
                            currDir=\$(pwd)
                            cd eea-deployer/product/jenkins/
                            ...
                            cd "\${currDir}"
                            tar czvf eea-deployer-\$VERSION_WITHOUT_HASH.tar.gz eea-deployer
                        """
                        // Upload the DEPLOYER package to the drop repository
                        arm.setUrl("https://arm.seli.gic.ericsson.se/", "$API_TOKEN_EEA")
                        arm.setRepo('proj-eea-drop-generic-local')
                        arm.deployArtifact('eea-deployer-$VERSION_WITHOUT_HASH.tar.gz', 'eea-deployer-$VERSION_WITHOUT_HASH.tar.gz')
                        currentBuild.description += '<br>eea-deployer drop package: <a href="https://arm.seli.gic.ericsson.se/artifactory/proj-eea-drop-generic-local/eea-deployer-' + "${VERSION_WITHOUT_HASH}" + '.tar.gz">' + "eea-deployer-${VERSION_WITHOUT_HASH}.tar.gz</a>"
                    }
                }
            }
        }
