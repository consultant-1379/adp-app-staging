...
pipeline {
    options {
        buildDiscarder(logRotator(daysToKeepStr: '7', artifactDaysToKeepStr: '7', artifactNumToKeepStr: '40'))
        skipDefaultCheckout()
    }
    agent {
        node {
  ...
  <tr>
    ...
    stages {
        ...
        stage('Init') {
            steps {
                script {
                    ...
                    env.PREPARE_UPGRADE_CUSTOM_CLUSTER_LABEL = params.UPGRADE_CLUSTER_LABEL
                }
            }
        }
        ...
        stage('Prepare upgrade') {
            when {
                expression {env.UPGRADE_CLUSTER_LABEL}
            }
            steps {
                script {
                    def prepare_upgrade_job = build job: "eea-product-prepare-upgrade", parameters: [
                        ...
                        stringParam(name: "CUSTOM_CLUSTER_LABEL", value: params.UPGRADE_CLUSTER_LABEL),
                ...
                catchError(stageResult: 'FAILURE', buildResult: 'FAILURE') {
                    script {
                        generic_upgrade_job = build job: "eea-common-product-upgrade", parameters: [
                            ...
                            stringParam(name: 'META_GERRIT_REFSPEC', value: params.META_GERRIT_REFSPEC),
