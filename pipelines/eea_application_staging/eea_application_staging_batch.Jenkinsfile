...
pipeline {
    ...
    parameters {
        ...
        string(name: 'CLUSTER_NAME', description: "Locked cluster with new nersion of CCD", defaultValue: '')
        string(name: 'CLUSTER_LABEL', description: "Cluster resource label to execute on. Default valuse is 'bob-ci'. Don't specify if CLUSTER_NAME specified", defaultValue: "${vars.resourceLabelCommon}")
...
<table>
  ...
  <tr>
    ...
    environment {
        ...
        UTF_DECISIVE_BATCH_STAGING_UTF_CUCUMBER_TESTS_EXECUTION_ID = "201" + "${env.BUILD_NUMBER}"
        ...
        UTF_DECISIVE_BATCH_STAGING_UTF_CUCUMBER_TESTS_TIMEOUT = "1800"
        ...
        stage('Checkout adp-app-staging') {
            steps {
                dir('adp-app-staging-full-checkout') {
                    ...
                    sh 'ln -s "${WORKSPACE}/adp-app-staging-full-checkout/technicals/pythonscripts" "${WORKSPACE}/adp-app-staging"'
                    sh 'ln -s "${WORKSPACE}/adp-app-staging-full-checkout/technicals/shellscripts" "${WORKSPACE}/shellscripts"'
                }
            }
        }
                ...
                stage('Lock') {
                    ...
                    stages {
                            ...
                            steps {
                                script {
                                    Map args = [
                                        ...
                                        "bobRulesetsPath":  "${WORKSPACE}/adp-app-staging-full-checkout/rulesets",
                                ...
                                script {
                                    withCredentials([usernamePassword(credentialsId: 'arm-eceaart-user-pass', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD'),
                                        ...
                                        withEnv(["CMA_EXT_CONFIG_FILE=${vars.cma_correlator_content_version_path}", "CMA_EXT_CONFIG_ENDPOINT=${vars.cma_correlator_content_version_endpoint}"]) {
                                            sh './bob/bob load-cm-analytics-config-cubes-into-eea > load_cm_analytics_config_cubes_into_correlator_content_version.log'
                                        }
                                        withEnv(["CMA_EXT_CONFIG_FILE=${vars.cma_config_path}"]) {
                                            sh './bob/bob load-cm-analytics-config-cubes-into-eea > load_cm_analytics_config_cubes_into_eea_all_in_one.log'
                                        }
                                    }
                                }
                            }
                                ...
                                always {
                                    script {
                                        ...
                                        if ("${currentBuild.result}" == 'FAILURE' ) {
                                            error ("Stage 'Run CMA health check after CM-Analytics config load' failed. " )
                                        }
                                    }
                                }
                            }
                        ...
                        stage('init UTF Test Variables') {
                            steps {
                                script {
                                    utf.initUtfTestVariables(vars, "adp-app-staging-full-checkout/technicals/utf_test_parameters.json")
                                }
                            }
                        }
                        ...
                        stage('UTF Post-activities') { // MUST BE SAME AS UTF_POST_ACTIVITIES_TEST_NAME
                            steps {
                                script {
                                    def utf_post_activities = [name: "${UTF_POST_ACTIVITIES_TEST_NAME}", build_result: 'SUCCESS', stage_result: 'FAILURE', exec_id: "${UTF_POST_ACTIVITIES_TEST_EXECUTION_ID}", metafilter: "${UTF_POST_ACTIVITIES_META_FILTER}", timeout: "${UTF_POST_ACTIVITIES_TEST_TIMEOUT}", logfile: "${UTF_POST_ACTIVITIES_TEST_LOGFILE}"]
                    ...
                    post {
                        always {
                            ...
                            script {
                                ...
                                sh "{ (echo '$system,${env.lock_start},${env.lock_end},eea-application-staging-batch' >> /data/nfs/productci/cluster_lock.csv) } || echo '/data/nfs/productci/cluster_lock.csv is unreachable'"
