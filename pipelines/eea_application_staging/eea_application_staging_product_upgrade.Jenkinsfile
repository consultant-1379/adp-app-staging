...
pipeline {
    ...
    parameters {
        ...
        string(name: 'CLUSTER_NAME', description: "Locked cluster with new version of CCD", defaultValue: '')
        string(name: 'CLUSTER_LABEL', description: "Special label for test new version of CCD", defaultValue: "${vars.resourceLabelUpgrade}")
        ...
        stage('Init') {
            steps {
                script {
                    ...
                    env.PREPARE_UPGRADE_CUSTOM_CLUSTER_LABEL = params.CLUSTER_LABEL
                }
            }
        }
            ...
            steps {
                script {
                    def prepare_upgrade_job = build job: "eea-product-prepare-upgrade", parameters: [
                        ...
                        stringParam(name: "CUSTOM_CLUSTER_LABEL", value: params.SPINNAKER_ID ? "${vars.resourceLabelUpgradePrefix}${JOB_NAME}-${BUILD_NUMBER}-${params.SPINNAKER_ID}" : params.CLUSTER_LABEL),
                ...
                catchError(stageResult: 'FAILURE', buildResult: 'FAILURE') {
                    script {
                        generic_upgrade_job = build job: "eea-common-product-upgrade", parameters: [
                            ...
                            stringParam(name: 'WAIT_FOR_CLUSTER_LOG_COLLECTOR_RESULT', value: params.WAIT_FOR_CLUSTER_LOG_COLLECTOR_RESULT),
                        ...
                        try {
                            downloadJenkinsFile("${env.JENKINS_URL}/job/eea-common-product-upgrade/${generic_upgrade_job.number}/artifact/artifact.properties")
                            archiveArtifacts artifacts: 'artifact.properties', allowEmptyArchive: true
                            def file_path = "${WORKSPACE}/artifact.properties"
                            readProperties(file: file_path).each {key, value -> env[key] = value }
                        }
                        catch (err) {
                            echo "Cought error during copy artifact.properties from eea-common-product-upgrade, err:${err}"
                        }
