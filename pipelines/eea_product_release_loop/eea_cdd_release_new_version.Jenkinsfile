...
pipeline {
    ...
    parameters {
        ...
        string(name: 'GIT_COMMENT', description: 'Comment for the version increase change', defaultValue: 'Increase version after PRA')
    }
    stages {
        ...
        stage('push patchset with PRA version') {
            environment {
                CDD_CHART_PATH="${WORKSPACE}/helm/eric-eea-cdd/Chart.yaml"
            }
            steps {
                script {
                    ...
                    def data = readYaml file: "${env.CDD_CHART_PATH}"
                    def old_version = data.version
                    def replace = "version: " + "${NEW_VERSION}"
                    sh """sed -i "s/version: $old_version/$replace/"  ${CDD_CHART_PATH}"""
                }
