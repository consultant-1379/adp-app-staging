  ...
  <tr>
    ...
    stages {
        ...
        stage('Validate patchset changes') {
            ...
            steps {
                script {
                    def result = gitmeta.verifyNoNewPathSet(params.GERRIT_REFSPEC)
                    if (!result) {
                ...
                catchError(stageResult: 'FAILURE', buildResult: 'FAILURE') {
                    script {
                        generic_upgrade_job = build job: "eea-common-product-upgrade", parameters: [
                            booleanParam(name: 'DRY_RUN', value: false),
                            stringParam(name: 'INT_CHART_NAME_META', value: params.INT_CHART_NAME),
                            stringParam(name: 'INT_CHART_REPO_META', value: params.INT_CHART_REPO),
                            stringParam(name: 'INT_CHART_VERSION_META', value: params.INT_CHART_VERSION),
                            ...
                            stringParam(name: 'SPINNAKER_ID', value: params.SPINNAKER_ID),
                            stringParam(name: 'PIPELINE_NAME', value: params.PIPELINE_NAME),
                            stringParam(name: 'META_GERRIT_REFSPEC',  value: params.GERRIT_REFSPEC),
                            ...
                            stringParam(name: 'ADP_APP_STAGING_GERRIT_REFSPEC_FOR_SF_INSTALL',  value: params.ADP_APP_STAGING_GERRIT_REFSPEC_FOR_SF_INSTALL),
                            stringParam(name: 'HELM_AND_CMA_VALIDATION_MODE', value: params.HELM_AND_CMA_VALIDATION_MODE)
        ...
        stage('Run robot results publisher') {
            when {
                expression {env.ARCHIVE_FILE_PATH && fileExists(env.ARCHIVE_FILE_PATH)}
            }
            steps {
