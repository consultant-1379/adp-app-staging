...
pipeline {
    ...
    triggers {
        gerrit (
            ...
            gerritProjects: [[
                ...
                filePaths: [[ compareType: 'ANT', pattern: 'jobs/jenkins_docker/**' ]]
            ]],
            triggerOnEvents:  [
                [
    ...
    stages {
        ...
        stage('Stage Job DSL generation and Job DSL content validation'){
            steps {
                jobDsl targets :'adp-app-staging/jobs/jenkins_docker/*.groovy'
                script {
                    try{
                        sh(
                            script: "${WORKSPACE}/adp-app-staging/technicals/shellscripts/collect_job_names.sh  ${WORKSPACE}/adp-app-staging/jobs/jenkins_docker"
                        )
