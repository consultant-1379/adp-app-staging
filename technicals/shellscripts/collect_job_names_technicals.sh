...
mapfile -t array < <( find technicals/ \(-name '*.groovy' -o -name  '*.Jenkinsfile'\) -print0 | xargs -0 git diff-tree --no-commit-id --name-only -r HEAD --diff-filter=ACMRT --)
...
do
    ...
    then
        # shellcheck disable=SC2086
        mapfile -t resultArray < <(cd ${REPO_PATH} && grep "${file}"  --include=\*.groovy -l -r ./technicals/)
