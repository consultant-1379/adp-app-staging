...
pipeline {
    ...
    triggers {
        gerrit (
            ...
            gerritProjects: [[
                ...
                filePaths: [[ compareType: 'ANT', pattern: 'jobs/eea_adp_staging/**' ]]
            ]],
            triggerOnEvents:  [
                [
    ...
    stages {
        ...
        stage('Stage Job DSL generation and Job DSL content validation'){
            steps {
                dir('adp-app-staging'){
                    jobDsl targets : 'jobs/eea_adp_staging/*.groovy'
                    script {
                        try{
                            sh(
                                script : "technicals/shellscripts/collect_job_names.sh jobs/eea_adp_staging"
                            )
