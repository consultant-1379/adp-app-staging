...
pipeline {
    ...
    environment {
        CONFLUENCE_ANCESTOR= 'Auto-generated documentation from cnint'
    ...
    stages {
            ...
            steps {
                script {
                    if ( env.GERRIT_REFSPEC ) {
                        ...
                        gitcnint.checkoutRefSpec("master", "FETCH_HEAD", 'cnint')
                    }
                }
            }
        }
        ...
        stage('adp-app-staging Checkout') {
            steps {
                script {
                    gitadp.checkoutRefSpec("master", "FETCH_HEAD", 'adp-app-staging')
                }
            }
        }
                        ...
                        def errors = []
                            ...
                            if (files) {
                                for (filename in files) {
                                    ...
                                    try {
                                        def gitiles_link = "https://${GERRIT_HOST}/plugins/gitiles/EEA/cnint/+/refs/heads/master/" + filename
