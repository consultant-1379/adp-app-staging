...
pipeline {
    ...
    triggers {
        gerrit (
            ...
            gerritProjects: [[
                ...
                filePaths: [[ compareType: 'ANT', pattern: 'jobs/**/*.groovy' ]]
            ]],
            triggerOnEvents:  [
                [
    ...
    stages {
        ...
        stage('Stage Job DSL generation and Job DSL content validation') {
            steps {
                script {
                    try{
                        sh(
                            script: "(cd 'adp-app-staging' && git diff-tree  --no-commit-id --name-only -r HEAD --diff-filter=ACMRT -- jobs/**/*.groovy )| { grep -v '^jobs/dslTest/' || true; } > env_inject"
                        )
        ...
        stage('Stage validating Jenkinsfile existence') {
            steps {
                script {
                    ...
                    try{
                        ...
                        sh "echo ${result}"
                        if (result != 0){
                            def wrongFile = readFile("adp-app-staging/err_inject")
                            echo "Jenkins file existence check FAILED : ${wrongFile}"
                            currentBuild.result = 'FAILURE'
                        }
                    }catch (err) {
                        echo "Caught: ${err}"
