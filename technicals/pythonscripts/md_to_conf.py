...
# link to the source : https://github.com/RittmanMead/md_to_conf
# --------------------------------------------------------------------------------------------------
# Usage: python md_to_conf.py markdown spacekey -u username -p password -o orgname
# --------------------------------------------------------------------------------------------------
"""
...
PARSER.add_argument('-w', '--warningmsg', action='store', default='',
                    help='Specify a warning message to display at the top'
                         'of the converted Confluence page.')
...
try:
    ...
    WARNING_MESSAGE = ARGS.warningmsg
    ...
    if headers:
        ...
        if links:
            for link in links:
                ...
                if result_ref:
                    base_uri = '%s/spaces/%s/pages/%s/%s' % (CONFLUENCE_API_URL,
                                                             SPACE_KEY,
                                                             page_id,
                                                             '+'.join(title.split()))
                    if VERSION == 1:
...
def create_page(title, body, ancestors):
    ...
    if response.status_code == 200:
        ...
        with open('CONFLUENCE_PAGE', 'w') as conffile:
            conffile.write(link)
            conffile.close()
...
def main():
    ...
    with codecs.open(MARKDOWN_FILE, 'r', 'utf-8') as mdfile:
        modified_md = mdfile.read()
        for match in re.finditer(r'(\[//\]:#.)\((.*)\)', modified_md):
            md_meta = match.group(2).split(':', 1)
            meta_dict[md_meta[0]].append(md_meta[1])
        modified_md = re.sub(r'<(?=[^!])', '< ', modified_md)
        modified_md = re.sub(r'(?<=[^-])>', ' >', modified_md)

        html = markdown.markdown(modified_md,
                                 extensions=['markdown.extensions.tables',
                                             'markdown.extensions.fenced_code'])

    if "confluence" in meta_dict:
        if meta_dict["confluence"][0] == "no":
            LOGGER.info('No conversion and Confluence update will be made because '
                        'of metadata confluence:no present')
            sys.exit(0)
    ...
    if "parentpage" in meta_dict:
        parent_page = get_page(meta_dict["parentpage"][0])
    else:
        if ANCESTOR:
