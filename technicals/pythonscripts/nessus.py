...
def get_report_results(srid):
    ...
    """
    scan_result_id = 'Scan Result ID: ' + str(srid)
    print(' {}'.format(scan_result_id))

    # TODO add timeout
    number_of_files = 0
    report_results = {}
    while number_of_files <= 2:
        data = connect('GET', '/rest/report')
        ...
        for key, value in report_ids.items():
            ...
            if status == 'Completed':
                ...
                if scan_result_id in str(value[0]):
                    report_results[value[1]] = key
                    number_of_files += 1
...
def execute(command, mask_token=None):
    """
    ...
    http_code = int(open('tmp_file', 'r').read())
    os.remove('tmp_file')
...
def check_if_already_scanned(arm_api_token, dst_full_paths):
    ...
    for dst_full_path in dst_full_paths:
        cmd = 'curl -H "X-JFrog-Art-Api: {0}" {1} -o /dev/null -s -w "%{{http_code}}" > tmp_file'.format(arm_api_token,
                                                                                                         dst_full_path)
        exec_status, http_code = execute(cmd, arm_api_token)
...
def do_scan(scan, service):
    ...
    if scan['scan_name'] == 'satellite':
        ...
        if scan_status != 'Completed':
            ...
            scan_completed = True
            print('\n\n Exporting the completed scan...')
            report_results = get_report_results(scan_result_id)
            print(' > reportresults = {}'.format(report_results))
            for output_file in output_files:
                if output_file.endswith('pdf'):
                    file_id = report_results['pdf']
                else:
                    file_id = report_results['csv']
...
if __name__ == '__main__':
    ...
    parser.add_argument('-arm', '--artifactory', help='Artfactory Manager Server', required=True)
    parser.add_argument('-at', '--armtoken', help='Artfactory Manager Token', required=True)
    ...
    scan['scanzone'] = config.get(scan['scan_name'], 'scanzone', fallback=0)
