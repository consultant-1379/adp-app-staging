...
pipeline {
    ...
    triggers {
        gerrit (
            ...
            gerritProjects: [[
                ...
                filePaths: [[ compareType: 'ANT', pattern: 'jobs/eea_product_ci_code_loop/**' ]]
            ]],
            triggerOnEvents:  [
                [
    ...
    stages {
        ...
        stage('Stage Job DSL generation and Job DSL content validation'){
            steps {
                jobDsl targets :'adp-app-staging/jobs/eea_product_ci_code_loop/*.groovy'
                script {
                    try{
                        sh(
                            script: "${WORKSPACE}/adp-app-staging/technicals/shellscripts/collect_job_names.sh  ${WORKSPACE}/adp-app-staging/jobs/eea_product_ci_code_loop"
                        )
