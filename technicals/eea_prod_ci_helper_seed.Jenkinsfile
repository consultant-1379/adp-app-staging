...
pipeline {
    ...
    triggers {
        gerrit (
            ...
            gerritProjects: [[
                ...
                filePaths: [[ compareType: 'ANT', pattern: 'jobs/eea_prod_ci_helper/**' ]]
            ]],
            triggerOnEvents:  [
                [
    ...
    stages {
        ...
        stage('Stage Job DSL generation and Job DSL content validation'){
            steps {
                dir('adp-app-staging'){
                    jobDsl targets : 'jobs/eea_prod_ci_helper/*.groovy'
                    script {
                        try{
                            sh(
                                script : "technicals/shellscripts/collect_job_names.sh jobs/eea_prod_ci_helper"
                            )
