                ...
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'test-jenkins-token', usernameVariable: 'TEST_USER', passwordVariable: 'TEST_USER_PASSWORD')]){
                        def result_next_build = sh(
                            script: "curl --silent ${TEST_JOB_URL}/api/json --user $TEST_USER:$TEST_USER_PASSWORD --insecure | grep -Po '\"nextBuildNumber\":\\K\\d+'",
                            returnStdout : true
                        )
                ...
                timeout(time: 5, unit: 'MINUTES'){
                    script{
                        ...
                        while ( "${env.TEST_STATE}" != "${EXCPECTED_RESULT}") {
                            ...
                            {
                                sh """
                                    curl '${TEST_JOB_URL}/${NEXT_BUILD_NUMBER}/api/json'  --user ${TEST_USER}:${TEST_USER_PASSWORD} --insecure | grep -Po '\"result\":\\s*\"\\K\\w+' | tee env_jobResult
                                    cat  env_jobResult
                                """
